...
if ($db) {
    ...
    $qry = mysqli_query($rplink, $sql);
    //while ($c = mysql_fetch_array($qry)) {
    while ($c = $qry->fetch_array(MYSQLI_ASSOC)) {
    ...
    while ($b = $qry->fetch_array(MYSQLI_ASSOC)) {
        $beeritem = array(
            "id" => $b['id'],
            "beername" => $b['name'],
            "style" => $b['style'],
	...
	<div class="bodywrapper">
		...
		<div class="header clearfix">
			<div class="HeaderLeft">
					<?php if($config[ConfigNames::UseHighResolution]) { ?>			
						<a href="admin/admin.php"><img
					src="<?php echo $config[ConfigNames::LogoUrl]; ?>" height="200"
					alt=""></a>
					<?php } else { ?>
						<a href="admin/admin.php"><img
					src="<?php echo $config[ConfigNames::LogoUrl]; ?>" height="100"
					alt=""></a>
					<?php } ?>
                </div>
			...
			<thead>
				<tr>
						<?php if($config[ConfigNames::ShowTapNumCol]){ ?>
							<th class="tap-num">TAP<br>#
					</th>
						<?php } ?>
						
						<?php if($config[ConfigNames::ShowSrmCol]){ ?>
							<th class="srm">GRAVITY
						<hr>COLOR
					</th>
						<?php } ?>
						
						<?php if($config[ConfigNames::ShowIbuCol]){ ?>
							<th class="ibu">BALANCE
						<hr>BITTERNESS
					</th>
						<?php } ?>
						
						<th class="name">BEER NAME &nbsp; &amp; &nbsp; STYLE
						<hr>TASTING NOTES
					</th>
						
						<?php if($config[ConfigNames::ShowAbvCol]){ ?>
							<th class="abv">CALORIES
						<hr>ALCOHOL
					</th>
						<?php } ?>
...
for ($i = 1; $i <= $numberOfTaps; $i ++) {
        if (isset($beers[$i])) {
            ...
            ?>
					...
					id="<?php echo $beer['id']; ?>">
								<?php if($config[ConfigNames::ShowTapNumCol]){ ?>
									<td class="tap-num"><span class="tapcircle"><?php echo $i; ?></span>
					</td>
								<?php } ?>
                ...
                ?> 
						...
						<div class="ibu-container">
							<div class="ibu-indicator">
								<div class="ibu-full" style="height:<?php echo $beer['ibu'] > 100 ? 100 : $beer['ibu']; ?>%"></div>
							</div>
                ...
                do {
                    ...
                    ?><div class="abv-indicator">
								<div class="abv-full" style="height:<?php echo $level; ?>%"></div>
							</div><?php
                ...
                ?>
						...
						<div class="ibu-container">
							<div class="ibu-indicator">
								<div class="ibu-full" style="height: 0%"></div>
							</div>
						</div>
						<h2></h2>
					</td>
						...
						<div class="abv-container">
							<div class="abv-indicator">
								<div class="abv-full" style="height: 0%"></div>
							</div>
						</div>
						<h2></h2>
					</td>
						...
						<div class="keg-container">
							<div class="keg-indicator">
								<div class="keg-full keg-empty" style="height: 0%"></div>
							</div>
						</div>
						<h2>0 fl oz left</h2>
					</td>
